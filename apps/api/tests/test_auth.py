...
def test_local_register_hashes_password_and_login_token_reaches_auth_me(db_override: None) -> None:
    registered = client.post(
        "/api/v1/auth/register",
        json={"email": "  LocalUser@Example.COM ", "password": "correct horse battery staple"},
    )
    assert registered.status_code == 201
    body = registered.json()
    assert body["user"]["email"] == "localuser@example.com"
    assert "password" not in body
    assert "password_hash" not in body

    engine = create_engine(TEST_DB_URL)
    with Session(engine) as session:
        stored = session.scalar(select(m.User).where(m.User.email == "localuser@example.com"))
        assert stored is not None
    ...
    assert me.json()["email"] == "localuser@example.com"


def test_local_duplicate_and_wrong_password_use_safe_errors(db_override: None) -> None:
    ...
    assert wrong.json()["detail"] == "authentication failed"
